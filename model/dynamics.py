...
class mlp(object):
    ...
    def affine_forward(self,layer_input, size ,layer_name):
        layer_out = tf.layers.dense(layer_input, size, activation= None, name=layer_name)
        with tf.variable_scope(layer_name, reuse=True):
            weight = tf.get_variable("kernel")
        cache=(layer_input, weight)
        return layer_out, cache, tf.nn.l2_loss(weight)
    ...
    def relu_backward(self, dout, cache):
        ...
        x = cache
        dx= dout*tf.where(x>0, tf.ones(tf.shape(x)),tf.zeros(tf.shape(x)))
        return dx
    ...
    def __call__(self, input, reuse=False):
        ...
        with tf.variable_scope(self.scope, reuse=reuse):
            ...
            for layer_i in range(self.n_layers):
                ...
                a, cache_a, layer_l2_loss = self.affine_forward(out,self.size, layer_name)

                l2_loss += layer_l2_loss
                r, cache_r = self.relu_forward(a)

                f_layer_output.append(a)
                f_layer_cache.append(cache_a)
                f_act_layer_output.append(r)
                f_act_layer_cache.append(cache_r)
                out = r

            out, cache_out, _ = self.affine_forward(out, self.output_size,"Output-Layer")
            dout =  tf.linalg.diag(np.ones((out.shape[-1], ),dtype=np.float32))
            dout = self.affine_backward(dout, cache_out)
            
            for layer_j in range(self.n_layers-1, -1, -1):
                dout = self.relu_backward(dout, f_act_layer_cache[layer_j])
                dout = self.affine_backward(dout, f_layer_cache[layer_j])
...
class NNDynamicsModel(DynamicsModel):
    def __init__(self,
        ...
        mse, l2_loss, self.predicted_unnormalized_deltas, self.normalized_jacobian_matrix = self._get_loss()
    ...
    def jacobian(self, states, actions):
        ...
        normalized_jacobian_matrix, std_deltas,std_obs,std_acts= self.sess.run(
            [self.normalized_jacobian_matrix,self.normalization.std_deltas,
            self.normalization.std_obs, self.normalization.std_acts,
            ],
            feed_dict=feed_dict)


        std_obs_acts= np.concatenate([1/(std_obs+self.epsilon),1/(std_acts+self.epsilon)])

        std_obs_acts= np.diag(std_obs_acts)

        std_deltas=np.diag(std_deltas+ self.epsilon)
    ...
    def _get_loss(self):
        ...
        predicted_normalized_deltas, l2_loss, normalized_jacobian_matrix = self.mlp(normalized_obs_and_acts, reuse=tf.AUTO_REUSE)
